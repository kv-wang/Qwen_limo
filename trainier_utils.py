...
def _create_adamuon_optimizer(
    model: "PreTrainedModel",
    training_args: "TrainingArguments",
) -> "torch.optim.Optimizer":
    from ..third_party.adamuon import Adamuon
    optimizer = Adamuon(
    ...
    logger.info_rank0("Using AdamUon optimizer.")
    return optimizer

def _create_limo_optimizer(
    model: "PreTrainedModel",
    training_args: "TrainingArguments",
) -> "torch.optim.Optimizer":
    from ..third_party.limo import LiMo
    ...
    logger.info_rank0("Using LiMo optimizer.")
    return optimizer
...
def create_custom_optimizer(
    ...
    if finetuning_args.use_adamuon:
        return _create_adamuon_optimizer(model, training_args)
    
    if finetuning_args.use_limo:
        return _create_limo_optimizer(model, training_args)
